...
/**
 * Variant of prettyHTML but with text output
 * @param diffs
 * @returns {string}
 */
diff_match_patch.prototype.diff_prettyText = function (diffs) {
    var text = [];
    var i = 0;
    for (var x = 0; x < diffs.length; x++) {
        var op = diffs[x][0];    // Operation (insert, delete, equal)
        ...
        switch (op) {
            case DIFF_INSERT:
                text[x] = '### INSERT #' + data + '# /INSERT ###';
                break;
            case DIFF_DELETE:
                text[x] = '### DELETE #' + data + '# /DELETE ###';
                break;
            case DIFF_EQUAL:
                text[x] = data;
                break;
...
export var DiffMatchPatch = diff_match_patch;