...
function avail(r: VdRenderer, data, selectedItinerary, selectedFare, visible) {
    ...
    % if (visible) {
        <div a:class="container availability">
...
function fareFamilyHeaders(r: VdRenderer, fareFamilies) {
    ...
    <div a:class="fare-family-names">
        ...
        <div a:class="fare-family-description">
        % for (let fareFamily of fareFamilies) {
...
function itineraryLine(r: VdRenderer, itinerary, selectedFare, showFareDetails: boolean, fareFamiliesList, jqFareFamilies, fareFamiliesCaveats) {
    ...
    <div a:class="itinerary">
        <div a:class="itinerary-header">
            <div a:class="itinerary-info right-delimiter">
                ...
                % }
                <strong a:class="total-duration">Total duration {{itinerary.duration}}</strong>
            </div>
            ...
            % for (let index=0;fareFamiliesList.length>index;index++) {
                ...
                <div [a:class]=clsList attr:style=("border-color:"+fare.color) onclick()=toggleFareDetails(index,itinerary)> 
                    % if (hasRecommendation(itinerary,fare)) {
                        % if (itinerary.isJQOnlyFlight && !fare.isMarginal) {
                            <span>
                                ...
                                <div a:class="jq-fare-name">{{jqFareFamilies[fare.code]? jqFareFamilies[fare.code].name : ""}}</div>
                            </span>
                        % }
                        % if (!(fare.isBusiness && !itinerary.flight.hasBusinessCabin)) {
                            <span a:class="fare-amount as-link">
                                % if (fare.isMarginal) {
                        ...
                        % } else {
                            <span a:class="no-seats">N/A</span>
                        % }
                    % } else {
                        <span a:class="no-seats">No seats</span>
                    % }

                    % if (lastSeatsAvailable(itinerary, fare)) {
                        <div a:class="last-seats">5 or fewer seats</div>
                    % }
                </div>
            % }
        </div>
...
function flightSummary(r: VdRenderer, departureAirport, departureTime, arrivalAirport, arrivalTime, airline, flightNumber, flightDuration, nbrOfStops) {
    ...
    <div a:class="flight-summary">
        ...
        <footer>
            <div a:class="flight-number as-link with-icon" onclick()=flightDetails()>
                <img a:class="icon" src=("https://book.qantas.com.au/go/2017.3-8/airlinesicons/"+airline.code.toLowerCase()+".png") width="14" height="14"/>
                {{airline.code.toUpperCase()}}{{flightNumber}}
            </div>
            <div a:class="flight-stops as-link">{{nbrOfStops}} stop(s)</div>
            <div a:class="flight-duration">{{flightDuration}}</div>
        </footer>
    </div>
     ---`
}
...
function fareDetailGroup(r: VdRenderer, flight, selectedFare, fareFamilies, jqFareFamilies, fareFamiliesCaveats) {
    ...
    <div a:class="itinerary-details">
        <div a:class="avail-actions">
            ...
            <div a:class="action-desc">
                <img a:class="icon" src="https://book.qantas.com.au/go/2017.3-8/fare-conditions-icons/teaser-earnpoints.svg" /> 
                Qantas Points earned 
            </div>
            <div a:class="action-desc">
                <img a:class="icon" src="https://book.qantas.com.au/go/2017.3-8/fare-conditions-icons/teaser-statuscredits.svg" /> 
                Status Credits earned
            </div>

            % for (let rule of fareFamilies[0].teaserRules) {
                <div a:class="action-desc">
                    <img a:class="icon" src=("https://book.qantas.com.au/go/2017.3-8/fare-conditions-icons/teaser-"+rule.ruleId.toLowerCase()+".svg") />
                    {{rule.label}}
...
function fareDetails(r: VdRenderer, fare, recommendation, isSelected) {
    ...
    <div [a:class]=clsList attr:style=("border-color:"+fare.color)>
            ...
            <div>
                ...
                % } else {
                    % if (!!rule.booleanValue) {
                        <div a:class="rule-yes"> v </div> // &#x2714;
                    % } else {
                        % if (rule.booleanValue == null) {
                            -
                        % } else {
                            <div a:class="rule-no"> x </div> // &#x2718;
                        % }
                    % }
                % }
            </div>
        % }
        <button a:class="btn-link full-fare"> Full fare conditions </button>
    </div>
     ---`
}
